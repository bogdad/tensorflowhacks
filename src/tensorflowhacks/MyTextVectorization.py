################## this is a backport of text vectorization load fix for keras 2.6
...
class MyTextVectorization(base_preprocessing_layer.PreprocessingLayer):
    ...
    This example instantiates a `MyTextVectorization` layer that lowercases text,
    ...
    >>>
    >>> # Create the layer.
    >>> vectorize_layer = tf.keras.layers.MyTextVectorization(
    ...
    This example instantiates a `MyTextVectorization` layer by passing a list
    ...
    def __init__(self,
        ...
        if "dtype" in kwargs and kwargs["dtype"] != tf.string:
            raise ValueError("MyTextVectorization may only have a dtype of string.")
        ...
        layer_utils.validate_string_arg(
                ...
                layer_name="MyTextVectorization",
        ...
        if output_mode == "binary":
            output_mode = self.MULTI_HOT
        if output_mode == "tf-idf":
            output_mode = self.TF_IDF
        # 'output_mode' must be one of (None, INT, COUNT, MULTI_HOT, TF_IDF)
        layer_utils.validate_string_arg(
                output_mode,
                allowable_strings=(self.INT, self.COUNT, self.MULTI_HOT, self.TF_IDF),
                layer_name="MyTextVectorization",
        ...
        if (output_mode == self.INT and not (isinstance(output_sequence_length, int) or
        ...
        if output_mode != self.INT and output_sequence_length is not None:
        ...
        base_preprocessing_layer.keras_kpl_gauge.get_cell("MyTextVectorization").set(
                True)
        ...
        self._index_lookup_layer = string_lookup.StringLookup(
                ...
                mask_token="",
                output_mode=output_mode if output_mode is not None else self.INT)

    def compute_output_shape(self, input_shape):
        if self._output_mode == self.INT:
    ...
    def compute_output_signature(self, input_spec):
        ...
        output_dtype = (tf.int64 if self._output_mode == self.INT
                                        else backend.floatx())
    ...
    def get_config(self):
        ...
        base_config = super(MyTextVectorization, self).get_config()
    ...
    def build(self, input_shape):
        ...
        if self._split is not None:
            if input_shape.ndims > 1 and not input_shape[-1] == 1:  # pylint: disable=g-comparison-negation
                raise RuntimeError(
                        "When using MyTextVectorization to tokenize strings, the innermost "
                        "dimension of the input array must be 1, got shape "
                        "{}".format(input_shape))

        super(MyTextVectorization, self).build(input_shape)
    ...
    def _preprocess(self, inputs):
        if self._standardize == LOWER_AND_STRIP_PUNCTUATION:
            ...
            raise ValueError(("%s is not a supported standardization. "
                                                "MyTextVectorization supports the following options "
